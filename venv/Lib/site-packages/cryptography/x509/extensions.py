...
class BasicConstraints(object):
    ...
    def __init__(self, ca, path_length):
        ...
        if not (path_length is None or ca):
...
class DistributionPoint(object):
    def __init__(self, full_name, relative_name, reasons, crl_issuer):
        ...
        if reasons and not (
            isinstance(reasons, frozenset)
            and all(isinstance(x, ReasonFlags) for x in reasons)
        ):
...
class TLSFeature(object):
    ...
    def __init__(self, features):
        ...
        if not all(isinstance(x, TLSFeatureType) for x in features) or not features:
            raise TypeError(
