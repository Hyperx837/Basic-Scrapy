...
class URL(object):
    ...
    def __init__(self, scheme=None, host=None, path=(), query=(), fragment=u'',
        ...
        if self._scheme and not _SCHEME_RE.match(self._scheme):
    ...
    def authority(self, with_password=False, **kw):
        ...
        host = self.host
        hostport = ['[' + host + ']'] if ':' in host else [self.host]
    ...
    def __eq__(self, other):
        ...
        for attr in ['scheme', 'userinfo', 'host', 'query',
            ...
            if getattr(self, attr) != getattr(other, attr):
                return False
        return self.path == other.path or (self.path in _ROOT_PATHS
    ...
    def remove(self, name, value=_UNSET, limit=None):
        ...
        if limit is None:
            if value is _UNSET:
                ...
                nq = [(k, v) for (k, v) in self.query if k != name or v != value]
        else:
            nq, removed_count = [], 0
...
def parse(url, decoded=True, lazy=False):
    ...
    if not decoded:
        return enc_url
    return DecodedURL(enc_url, lazy=lazy)
